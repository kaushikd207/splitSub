...
Deploy the web app as static assets and run the API behind HTTPS with a managed PostgreSQL and Redis instance. Configure `WEB_ORIGIN`, `DATABASE_URL`, `REDIS_URL`, a high-entropy `JWT_SECRET`, and `PAYMENT_WEBHOOK_SECRET` in the server environment. Set `NODE_ENV=production` so session cookies are `Secure`.
